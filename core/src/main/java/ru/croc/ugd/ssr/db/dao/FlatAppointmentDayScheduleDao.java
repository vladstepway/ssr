...
/**
 * DAO for FlatAppointmentDayScheduleDao.
 */
public interface FlatAppointmentDayScheduleDao extends Repository<DocumentData, String> {

    @Query(value = "select * from documents"
        + " where doc_type = 'FLAT-APPOINTMENT-DAY-SCHEDULE'"
        + " and (cast(cast(:from as text) as date) is null"
        + "   or cast(json_data -> 'flatAppointmentDaySchedule' -> 'flatAppointmentDayScheduleData'"
        ...
        + "   or json_data -> 'flatAppointmentDaySchedule' -> 'flatAppointmentDayScheduleData'"
    ...
    @Query(value = "SELECT DISTINCT ON (id) docs.* FROM documents docs, "
        + " jsonb_array_elements(docs.json_data -> 'flatAppointmentDaySchedule'"
        + " -> 'flatAppointmentDayScheduleData' -> 'slots' "
        + " -> 'slot') as slot, "
        + " jsonb_array_elements(slot -> 'windows' -> 'window') AS windows "
        + " WHERE doc_type = 'FLAT-APPOINTMENT-DAY-SCHEDULE' AND windows ->> 'preBookingId' = :preBookingId"
        + "     AND cast((cast(docs.json_data -> 'flatAppointmentDaySchedule' -> 'flatAppointmentDayScheduleData' "
    ...
    @Query(value = "SELECT docs.* FROM documents docs, "
        ...
        + "     AND cast((cast(docs.json_data -> 'flatAppointmentDaySchedule' -> "
        + "         'flatAppointmentDayScheduleData' ->> 'date' as text)"
    ...
    @Query(value = "select exists (SELECT 1 FROM documents docs, "
        ...
        + " jsonb_array_elements(slot -> 'windows' -> 'window') AS windows "
        + " WHERE doc_type = 'FLAT-APPOINTMENT-DAY-SCHEDULE'"
        + " AND windows ->> 'preBookingId' = :preBookingId AND windows ->> 'preBookedUntil' is null"
        + " AND cast((cast(docs.json_data -> 'flatAppointmentDaySchedule' -> 'flatAppointmentDayScheduleData' "
        + "     ->> 'date' as text) || ' ' || cast(slot ->> 'timeFrom' as text)) as timestamp) > now())",
        nativeQuery = true)
