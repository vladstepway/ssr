...
public class PersonResponseListener
    ...
    public String getMessageType() {
        return mqetpmvProperties.getPersonResponseMessageType();
    }
    ...
    public IntegrationFlowType getIntegrationFlowType() {
        return IntegrationFlowType.DGI_TO_DGP_PERSON_RESPONSE;
    }

    @Override
    public void handle(String message) {
        final FlowReceivedMessageDto<SuperServiceDGPPersonsMessage> superServiceDgpPersonsMessage =
            toFlowReceivedMessageDto(message);
        if (nonNull(superServiceDgpPersonsMessage)) {
            personsUpdateFlowsService.updatePersonsInfo(superServiceDgpPersonsMessage);
        }
    }
     ...
     */
    private FlowReceivedMessageDto<SuperServiceDGPPersonsMessage> toFlowReceivedMessageDto(String message) {
        CoordinateSendTaskStatusesMessage statusMessage = parseMessage(message);

        CoordinateTaskStatusData coordinateTaskStatusDataMessage = statusMessage.getCoordinateTaskStatusDataMessage();
        final boolean shouldSendNotifications = !isPartOfAffairCollation(coordinateTaskStatusDataMessage);

        if (coordinateTaskStatusDataMessage.getStatus().getStatusCode() == 404) {
            log.warn(
                "По первому потоку поступил пустой ответ (с кодом 404), TaskId: {}",
                coordinateTaskStatusDataMessage.getTaskId()
            );
            return null;
        }

        final TaskResult.XmlView xmlView = coordinateTaskStatusDataMessage.getResult().getXmlView();
